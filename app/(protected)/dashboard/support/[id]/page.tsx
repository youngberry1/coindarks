...
export default async function UserTicketDetailPage({ params }: TicketDetailProps) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-4xl mx-auto">
            {/* Header */}
            <div>
                <Link href="/dashboard/support" className="inline-flex items-center text-sm text-foreground/50 hover:text-primary mb-4 transition-colors">
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                    <div>
                        ...
                        <h1 className="text-2xl md:text-3xl font-black tracking-tight mb-2">{ticket.subject}</h1>
                        ...
                        <div key={msg.id} className={`flex gap-4 ${!msg.is_admin_reply ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <div className={`max-w-[85%] rounded-[24px] p-6 ${!msg.is_admin_reply
                                ...
                                <div className="flex items-center justify-between gap-4 mb-2">
                                    <span className={`text-xs font-bold uppercase tracking-wider ${!msg.is_admin_reply ? 'text-primary' : 'text-foreground/50'}`}>
